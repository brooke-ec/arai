...
class InviteTracker(commands.Cog):
    ...
    async def on_invite_create(self, invite:discord.Invite):
        if invite.guild.id != self.client.watch_guild_id: return
    ...
    async def get_invite_used(self) -> InviteData | None:
        ...
        if len(diff) > 1: diff = {i for i in diff if i.expires_at is not None and i.expires_at.timestamp() > datetime.utcnow().timestamp()}
        if len(diff) == 1: return diff.pop()
        # If execution gets to here then must be through vanity invite
        return None

    @commands.Cog.listener()
    async def on_member_join(self, member:discord.Member):
        if member.guild.id != self.client.watch_guild_id: return
        # Get Invite
        invite = await self.get_invite_used()
        # Send Log Message
        if invite is None:
            LOGGER.info("`%s`'s invite could not be found")
            embed = discord.Embed(title=f"{member} Joined!", description=f"Could not determine the invite {member.mention} joined through", color=0x04ff00)
        ...
        if name is None:
            embed = discord.Embed(title=f"{member} Joined!", description=f"{member.mention} Joined through {invite.inviter.mention}'s invite", color=0x04ff00)
        else:
            embed = discord.Embed(title=f"{member} Joined!", description=f"{member.mention} Joined from invite **{name}**", color=0x04ff00)
        embed.set_thumbnail(url=member.avatar.url)
    ...
    @app_commands.command(name="create_invite", description="Creates a new unique invite for tracking where members join from")
    ...
    async def create_invite_command(self, interaction:discord.Interaction, channel:discord.TextChannel, name:str, role:Optional[discord.Role]):
        ...
        embed = discord.Embed(title=f"Invite ``{name}`` Created", color=0x00e5ff,
