...
class DynamicVoiceChannels(commands.Cog):
    ...
    @app_commands.command(name="set_visible", description="Sets the minimum number of voice channels that should be visible")
    ...
    async def set_min_command(self, interaction:discord.Interaction, number:int):
        if number > len(self.channels): 
            return await interaction.response.send_message("<:cross_red:768507522335506473> Cannot be larger than the number of channels", ephemeral=True)
        self.min_visible = number
        await interaction.response.send_message(f"<:universal_check:768906325010022411> Minimum visible channels set to **{number}**", ephemeral=True)
    ...
    async def on_voice_state_update(self, member:discord.Member, before:discord.VoiceState, after:discord.VoiceState):
        if True not in self.client.channels_in_guild(before.channel, after.channel): return
        if before.channel == after.channel: return
        
        b, a = before.channel is not None, after.channel is not None
        if b: self.get_channel(before.channel).connected -= 1
        if a: self.get_channel(after.channel).connected += 1
        if b or a: await self.update_visibility()
    ...
    def calculate_visibility(self) -> List[bool]:
        ...
        for i in range(self.min_visible): channels[i] = True # Make default channels visible (General I)
        return channels
