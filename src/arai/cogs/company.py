...
class TerraCompany(commands.Cog):
    ...
    async def on_voice_state_update(self, member:discord.Member, before:discord.VoiceState, after:discord.VoiceState):
        ...
        if not self.client.channels_in_guild(after.channel)[0]: return
        if member.id != self.client.following_id: return
    ...
    async def join_command(self, interation:discord.Interaction, member:Optional[discord.Member]):
        if member is None: member = interation.user
        if member.voice is None:
            await interation.response.send_message("<:cross_red:768507522335506473> That user is not connected to a voice channel", ephemeral=True)
    ...
    async def join(self, channel:discord.VoiceChannel):
        if channel.guild.voice_client is not None:
            if channel.guild.voice_client.channel == channel: return
