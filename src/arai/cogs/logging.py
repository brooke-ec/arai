...
class MiscLogging(commands.Cog):
    ...
    async def on_raw_member_remove(self, payload:discord.RawMemberRemoveEvent):
        if payload.guild_id != self.client.watch_guild_id: return
        time_here = datetime.now(timezone.utc) - payload.user.joined_at
        embed = discord.Embed(title=f"{payload.user} Left", description=f"{payload.user.mention} has left after {time_here.days} days", color=0xff0000)
        embed.set_thumbnail(url=payload.user.avatar.url)
        await self.client.send_log(embed=embed)

    @commands.Cog.listener()
    async def on_voice_state_update(self, member:discord.Member, before:discord.VoiceState, after:discord.VoiceState):
        if after.channel.guild.id != self.client.watch_guild_id: return
        ...
        if before.channel is not None:
            ...
            if member.id in self.join_records: self.join_records.pop(member.id)
